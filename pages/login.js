...
function login({ hideSidebar, host, proto }) {
  useEffect(() => {
    if (localStorage.getItem("token")) {
      router.push("/");
    }
  ...
  return (
    <section className="bg-[#4c6fc0]" onClick={hideSidebar}>
      <ToastContainer
        position="bottom-center"
        autoClose={5000}
        hideProgressBar={true}
      ...
      />
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto h-screen">
        <p className="flex items-center mb-4 text-4xl text-white font-medium">
          Welcome back!
        </p>
        <div className="flex">
          <p className="flex items-center mb-6 mr-2 text-md text-white font-normal">
            Don't have an account yet?
          </p>
          <div onClick={() => router.push("/register")}>
            <p className="flex cursor-pointer items-center mb-6 underline text-md text-white font-normal">
              Sign up
            </p>
          </div>
        </div>
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              <div>
                <label
                  htmlFor="email"
                  className="block mb-2 text-md font-normal text-white"
                >
                  Username
                </label>
                <input
                  ...
                  name="email"
                  id="email"
                  className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 block w-full p-2.5 "
              ...
              <div className="flex items-center justify-between">
                ...
                <a
                  href="#"
                  className="text-sm mobile:text-xs font-normal text-white underline"
                >
                  Forgot password?
                </a>
              </div>
              <button
                type="submit"
                className="w-full text-white bg-[#7a96d5] focus:ring-4 focus:outline-none font-semibold rounded-lg text-lg px-5 py-2.5 text-center"
              >
                Login
              </button>
            </form>
